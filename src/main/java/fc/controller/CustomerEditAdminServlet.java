 ...
 * @author william
 */
@WebServlet(name = "CustomerEditAdminServlet", urlPatterns = {"/CustomerEditAdminServlet"})
public class CustomerEditAdminServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (errors.isEmpty()) {
            ...
            customerDB.add(new Customer(ID, firstName, lastName, email, password, passport, DOB));

            response.sendRedirect("user_management.jsp");
        } else {
            //put errors in request scope and forward them back to register.jsp
            request.setAttribute("errors", errors);
            request.getRequestDispatcher("userDetail_management.jsp").forward(request, response);
        }
    }
